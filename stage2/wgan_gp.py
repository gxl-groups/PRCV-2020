...
class GoodGenerator(nn.Module):
    def __init__(self, dim=64,output_dim=64*64*1):
        super(GoodGenerator, self).__init__()

        self.dim = dim

        self.ln1 = nn.Linear(128+5, 4*4*8*self.dim)
        self.rb1 = ResidualBlock(8*self.dim, 8*self.dim, 3, resample = 'up')
        self.rb2 = ResidualBlock(8*self.dim, 4*self.dim, 3, resample = 'up')
        self.rb3 = ResidualBlock(4*self.dim, 2*self.dim, 3, resample = 'up')
        self.rb4 = ResidualBlock(2*self.dim, 2*self.dim, 3, resample = 'up')
        self.rb5 = ResidualBlock(2*self.dim, 1*self.dim, 3, resample = 'up')
    ...
    def forward(self, input, label):
        ...
        output = self.ln1(output.contiguous())
...
class GoodDiscriminator(nn.Module):
    def __init__(self, dim=64, input_size=64):
        ...
        self.rb5 = ResidualBlock(8*self.dim, 8*self.dim, 3, resample = 'down', hw=int(input_size/16))
    ...
    def forward(self, input):
        ...
        output = self.ln1(output)  # (bs, 1)
        output = output.view(-1)  # (bs, )
        return output

# ----------------------------------------------wgan patch D -----------------------------------------------------------
class GoodDiscriminator_Patch(nn.Module):
    def __init__(self, dim=64, input_size=64):
        super(GoodDiscriminator_Patch, self).__init__()
        ...
        self.rb4 = ResidualBlock(8*self.dim, 8*self.dim, 3, resample = 'down', hw=int(input_size/8))
        self.rb5 = ResidualBlock(8*self.dim, 8*self.dim, 3, resample = 'down', hw=int(input_size/16))
        self.convlast = MyConvo2d(8*self.dim, 1, 3, he_init = False)
    ...
    def forward(self, input):
        ...
        output = self.rb4(output)
        output = self.rb5(output)
        output = self.convlast(output)  # (bs, 1, 4, 4)
        output = output.view(-1)  # (bs*4*4, )
        return output
